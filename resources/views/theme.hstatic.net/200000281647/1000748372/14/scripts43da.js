...
function getCartModal(){
	...
	jQuery.getJSON('/cart.js', function(cart, textStatus) {
		if(cart) {
			...
			$(".modal-cart-mobile").show();
			setTimeout(() =>{
			$(".modal-cart-mobile").hide();
			},3000)
			...
			jQuery.each(cart.items,function(i,item){
				...
				if(item.image != null)
					...
					tr.find('.item-image').html("<img src='//theme.hstatic.net/200000281647/1000748372/14/no_image.jpg?v=242'>");
...
function clone_item(product,i){
	...
	if ( product.image == null ) {
		item_product.find('img').attr('src','//theme.hstatic.net/200000281647/1000748372/14/no_image.jpg?v=242').attr('alt', product.url);
	} else {
...
/*=======================================*/
jQuery(document).ready(function(){
	$(".background-opacity,.modal-cart-mobile .close").click(function(){
		$(".modal-cart-mobile").hide();
	})
